...
describe('ChatPage - Planning Presentation Lifecycle', () => {
  ...
  // ── Panel auto-open trigger ─────────────────────────────────

  it('first progress scaffold line sets planningToolOpened flag', () => {
    let planningToolOpened = false

    updatePlanProgressPresentation({ phase: 'analyzing', message: 'Analyzing task complexity...' })

    // Simulate tryOpenPlanningPanel behavior
    if (planPresentationSource.value === 'progress' && !planningToolOpened) {
      planningToolOpened = true
    }

    expect(planningToolOpened).toBe(true)
    expect(planPresentationText.value).toContain('# Planning...')
  })

  it('clearPlanPresentation resets planningToolOpened flag', () => {
    let planningToolOpened = true
    let planningToolPendingOpen = true

    clearPlanPresentation()
    planningToolOpened = false
    planningToolPendingOpen = false

    expect(planningToolOpened).toBe(false)
    expect(planningToolPendingOpen).toBe(false)
    expect(planPresentationText.value).toBe('')
  })

  it('defers planning panel auto-open while reasoning view is active', () => {
    let planningToolOpened = false
    let planningToolPendingOpen = false
    let chatViewMode: 'chat' | 'reasoning' = 'reasoning'
    let openCalls = 0

    const canOpenLiveViewPanel = () => chatViewMode !== 'reasoning'
    const showToolPanelIfAllowed = () => {
      if (!canOpenLiveViewPanel()) return false
      openCalls += 1
      return true
    }

    function tryOpenPlanningPanel() {
      if (planningToolOpened) return
      if (!canOpenLiveViewPanel()) {
        planningToolPendingOpen = true
        return
      }
      if (showToolPanelIfAllowed()) {
        planningToolOpened = true
        planningToolPendingOpen = false
      }
    }
    ...
    expect(planningToolOpened).toBe(false)
    expect(planningToolPendingOpen).toBe(true)
