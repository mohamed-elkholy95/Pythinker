...
describe('waitForSessionReady', () => {
  ...
  it('polls until session is no longer initializing', async () => {
    ...
    const promise = waitForSessionReady('session-1', getSessionFn, { pollIntervalMs: 50, maxWaitMs: 1000 })
  ...
  it('returns timedOut when still initializing after max wait', async () => {
    ...
    const promise = waitForSessionReady('session-1', getSessionFn, { pollIntervalMs: 50, maxWaitMs: 120 })
