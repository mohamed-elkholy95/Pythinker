...
const formatConsoleOutput = (records: ConsoleRecord[], maxLength: number): string => (
  truncate(
    records.map((record: ConsoleRecord) => {
      const ps1 = cleanPs1(record.ps1 || '');
      const prompt = ps1 ? `${ps1} ` : '$ ';
      const command = record.command || '';
      const output = cleanShellOutput(record.output || '', command);
...
const formatShellLikePreview = (toolContent: ToolContent, maxLength: number): string => {
  ...
  const stdout = stripCmdMarkers(String(toolContent.stdout || toolContent.content?.stdout || ''));
  const stderr = stripCmdMarkers(String(toolContent.stderr || toolContent.content?.stderr || ''));
