...
export function validateUserInput(data: {
  ...
  // Password presence validation only.
  // Password creation rules are enforced separately via the backend-owned password policy contract.
  if (data.password !== undefined) {
    if (!data.password) {
      errors.password = i18n.global.t('Password is required')
    }
  }
