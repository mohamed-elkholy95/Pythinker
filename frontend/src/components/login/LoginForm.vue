<template>
  <div class="w-full max-w-[384px] py-[24px] pt-0 px-[12px] relative" style="z-index:1">
    <div class="flex flex-col justify-center gap-[40px] text-[var(--text-primary)] max-sm:gap-[12px]">
      ...
      <!-- Email Verification Step (when login blocked for unverified user) -->
      <RegisterVerificationForm
        v-if="showEmailVerificationStep"
        :email="formData.email"
        :verification-state="emailVerificationState"
        @success="emits('success')"
        @back-to-form="resetEmailVerificationStep"
      />
...
import { getCachedAuthProvider, resendRegistrationCode, type VerificationState } from '@/api/auth'
...
const getLoginErrorMessage = (error: unknown): string => {
  ...
  if (error instanceof AxiosError) {
    raw = error.response?.data?.msg || raw
  }
  const lower = raw.toLowerCase()

  if (lower.includes('invalid email or password')) return t('Incorrect email or password. Please try again.')
  if (lower.includes('account is temporarily locked')) return t('Too many failed attempts. Your account is temporarily locked. Please try again later.')
  if (lower.includes('invalid totp') || lower.includes('invalid verification code')) return t('Invalid verification code. Please check and try again.')
  if (lower.includes('inactive')) return t('Your account has been deactivated. Please contact support.')

  return raw || t('Login failed. Please check your credentials and try again.')
}

// Form state
const showPassword = ref(false)
const showTotpStep = ref(false)
const showEmailVerificationStep = ref(false)
const emailVerificationState = ref<VerificationState | null>(null)
const totpCode = ref('')
...
const resetEmailVerificationStep = () => {
  showEmailVerificationStep.value = false
  emailVerificationState.value = null
}
...
const validateForm = () => {
  // Login only validates email format + password not empty (no complexity rules)
  const result = validateUserInput({ email: formData.value.email })
...
const handleSubmit = async () => {
  ...
  if (showTotpStep.value) {
    ...
    try {
      ...
      totpCode.value = ''
      showErrorToast(getLoginErrorMessage(err))
    }
    return
  }
  ...
  try {
    ...
    if (errorCode === 'email_not_verified') {
      try {
        emailVerificationState.value = await resendRegistrationCode({ email: formData.value.email })
        showEmailVerificationStep.value = true
      } catch {
        emailVerificationState.value = null
        showErrorToast(t('Your email is not verified but we could not send a verification code. Please try again.'))
      }
    } else {
