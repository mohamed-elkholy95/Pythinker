<template>
    <div v-if="promptDialogVisible" class="absolute z-[1000] pointer-events-auto">
        ...
        <div role="dialog"
            ...
            <div class="px-5 py-3">
                <input
                    ...
                    class="w-full px-3 py-2 text-sm border border-[var(--border-main)] rounded-[10px] bg-[var(--background-white-main)] text-[var(--text-primary)] placeholder-[var(--text-tertiary)] focus:outline-none focus:border-[var(--border-focus)] focus:ring-1 focus:ring-[var(--border-focus)]"
            ...
            <div class="flex justify-end gap-2 p-5">
                <button
                    ...
                    @click="handlePromptCancel">{{ promptDialogConfig.cancelText }}</button>
                <button
                    class="rounded-[10px] px-3 py-2 text-sm flex flex-row gap-1 items-center cursor-pointer transition-colors border border-[var(--border-btn-primary)] bg-[var(--button-primary)] text-[var(--text-white)] hover:bg-[var(--button-primary-hover)]"
                    @click="handlePromptConfirm">{{ promptDialogConfig.confirmText }}</button>
            </div>
        </div>
    </div>
</template>
...
const { promptDialogVisible, promptDialogConfig, promptInputValue, handlePromptConfirm, handlePromptCancel } = useDialog()
