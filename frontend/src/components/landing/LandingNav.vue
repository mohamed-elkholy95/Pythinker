  ...
  <nav :class="['landing-nav', { 'nav-scrolled': isScrolled }]">
    <div class="nav-inner">
      ...
      <a href="/" class="nav-logo" aria-label="Pythinker home">
        <img src="/logo.png" alt="" width="28" height="28" class="nav-logo-img" />
      ...
      <div class="nav-actions">
        <button class="theme-toggle" :aria-label="isDark ? 'Switch to light mode' : 'Switch to dark mode'"
          @click="toggleTheme">
...
.theme-toggle:hover { background: var(--fill-tsp-gray-main); color: var(--icon-primary); }
.nav-btn {
...
.nav-hamburger {
  ...
  background: transparent; border: none;
  color: var(--icon-primary); cursor: pointer; border-radius: 8px;
}
