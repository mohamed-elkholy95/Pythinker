<template>
  ...
  >
    ...
    <div v-if="isSearching" class="deal-container">
      ...
      <!-- Web-wide search indicator (no explicit store list) -->
      <div v-if="mergedStoreStatuses.length === 0 && isSearching" class="store-status-grid">
        <span class="store-chip store-chip--pending">
          <Loader2 :size="10" class="store-chip-icon spinning" />
          All Stores
        </span>
        <span class="store-chip store-chip--pending">
          <Loader2 :size="10" class="store-chip-icon spinning" />
      ...
      <!-- Store status grid (explicit store list) -->
      <div v-else-if="mergedStoreStatuses.length > 0" class="store-status-grid">
        <span
...
const storeProgressLabel = computed(() => {
  ...
  // Web-wide search (no explicit store list)
  if (total === 0) {
    const parts: string[] = ['Searching all stores'];
    if (dealCount > 0) parts.push(`${dealCount} deal${dealCount !== 1 ? 's' : ''} found`);
    return parts.join(' \u00b7 ');
  }
