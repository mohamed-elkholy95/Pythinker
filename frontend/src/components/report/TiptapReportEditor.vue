...
const htmlContent = computed(() => {
  ...
  const alertNormalized = normalizeInlineAlerts(linkedMarkdown);
  const collapsed = alertNormalized.replace(/\n{3,}/g, '\n\n');
...
/** Build a styled alert header from alert metadata (safe — only static trusted strings). */
...
const enhanceAlertBlockquotes = () => {
  ...
  for (const bq of blockquotes) {
    ...
    // Strategy 1: GitHub-flavored [!TYPE] syntax
...
/** Replace broken chart/report images with a styled placeholder instead of the browser's broken-image icon. */
const addImageFallbacks = () => {
  ...
  images.forEach((img: HTMLImageElement) => {
    ...
    const showPlaceholder = () => {
      ...
      placeholder.className = 'chart-image-fallback';
      placeholder.textContent = `Chart image unavailable: ${img.alt || 'image'}`;
      img.parentElement?.insertBefore(placeholder, img.nextSibling);
    };

    // Handle images that already failed before this runs (timing race)
...
/* ── Chart image fallback placeholder ─────────────────────────────── */
