...
class CodeFormatRequest(BaseModel):
    ...
    check_only: Optional[bool] = Field(
        default=False, description="Check formatting without modifying file"
    )
...
class CodeLintRequest(BaseModel):
    ...
    linter: Optional[Linter] = Field(
        default=Linter.AUTO, description="Linter to use (auto detects from file type)"
    )
    fix: Optional[bool] = Field(
        default=False, description="Automatically fix issues where possible"
    )
...
class CodeSearchRequest(BaseModel):
    ...
    file_glob: Optional[str] = Field(
        default="*", description="Glob pattern to filter files (e.g., '*.py', '*.js')"
    )
    context_lines: Optional[int] = Field(
        default=2, ge=0, le=10, description="Number of context lines before/after match"
    )
