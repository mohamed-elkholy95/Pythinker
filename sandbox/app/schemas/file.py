...
class FileReadRequest(BaseModel):
    ...
    max_length: Optional[int] = Field(
        10000, description="Maximum length of the content to return"
    )
...
class FileWriteRequest(BaseModel):
    ...
    leading_newline: Optional[bool] = Field(
        False, description="Whether to add leading newline"
    )
    trailing_newline: Optional[bool] = Field(
        False, description="Whether to add trailing newline"
    )
    sudo: Optional[bool] = Field(False, description="Whether to use sudo privileges")
...
class FileDeleteRequest(BaseModel):
    ...
    path: str = Field(..., description="Absolute path of file/directory to delete")
    sudo: Optional[bool] = Field(False, description="Whether to use sudo privileges")
