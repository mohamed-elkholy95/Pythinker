...
class TestRunRequest(BaseModel):
    ...
    coverage: Optional[bool] = Field(
        default=False, description="Whether to collect code coverage"
    )
    timeout: Optional[int] = Field(
        default=300, ge=10, le=1800, description="Test timeout in seconds (10-1800)"
    )
...
class CoverageReportRequest(BaseModel):
    ...
    output_format: Optional[str] = Field(
        default="html", description="Report format: 'html', 'xml', 'json'"
    )
    output_dir: Optional[str] = Field(
        None, description="Output directory for report (defaults to workspace output)"
    )
