...
class OrganizeRequest(BaseModel):
    ...
    target_category: Optional[FileCategory] = Field(
        default=FileCategory.OTHER, description="Target category for organization"
    )
...
class ArchiveRequest(BaseModel):
    ...
    base_path: Optional[str] = Field(
        None, description="Base path for files (defaults to workspace root)"
    )
...
class ReportRequest(BaseModel):
    ...
    output_format: Optional[ReportFormat] = Field(
        default=ReportFormat.MARKDOWN, description="Output format: markdown, html, json"
    )
    title: Optional[str] = Field(default="Workspace Report", description="Report title")
    include_sections: Optional[List[str]] = Field(
        None, description="Specific sections to include (optional)"
    )
