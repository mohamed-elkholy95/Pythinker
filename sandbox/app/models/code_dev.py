...
class LintResult(BaseModel):
    ...
    issues: List[LintIssue] = Field(default_factory=list, description="List of issues found")
    ...
    fixed_count: int = Field(default=0, description="Number of issues auto-fixed (if --fix)")
    message: Optional[str] = Field(None, description="Status message")
...
class AnalysisResult(BaseModel):
    ...
    security_issues: List[SecurityIssue] = Field(default_factory=list, description="Security issues found")
