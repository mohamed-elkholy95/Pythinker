...
| File | Change | Status |
|------|--------|--------|
| `backend/app/domain/services/agents/base.py` | Replace `SAFE_PARALLEL_TOOLS`, `PHASE_TOOL_GROUPS`, `search_functions` with `ToolName` enum methods | ⬜ Pending |
| `backend/app/domain/services/agents/execution.py` | Replace string tool checks in `_track_sources_from_tool_event`, `_track_multimodal_findings` | ⬜ Pending |
| `backend/app/domain/services/tools/dynamic_toolset.py` | Register tools using `ToolCapability` | ⬜ Pending |
