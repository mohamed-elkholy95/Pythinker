...
class GridFSFileStorage(FileStorage):
    ...
    async def upload_file(
        ...
        try:
            ...
            file_id = await bucket.upload_from_stream(filename, file_data, metadata=file_metadata)
            ...
            logger.error(f"Failed to upload file {filename} for user {user_id}: {e!s}")
            raise
    ...
    async def download_file(self, file_id: str, user_id: str | None = None) -> tuple[BinaryIO, FileInfo]:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Invalid file ID format: {file_id}") from e
