...
class APIKeyPool:
    ...
    def __init__(
        ...
        # Flag for lazy Redis acquisition — if the factory couldn't provide
        # a Redis client at init time (sync/async boundary), we'll try again
        # on the first async operation when an event loop is available.
        self._redis_init_attempted = redis_client is not None
        if redis_client is None:
            logger.info(
                f"[{provider}] APIKeyPool starting without Redis — will attempt lazy acquisition on first async use."
            )

    async def _ensure_redis(self) -> None:
        """Lazily acquire a Redis client if not available at init time.

        The LLM factory creates APIKeyPool under threading.Lock (sync context),
        so ``get_redis()`` may fail if it requires an async event loop.
        This method retries acquisition on the first async call, bridging
        the sync/async gap.  Called once; subsequent calls are no-ops.
        """
        if self._redis is not None or self._redis_init_attempted:
            return
