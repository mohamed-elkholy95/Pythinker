...
class MinIOStorage:
    ...
    async def delete_screenshot_objects(self, object_keys: list[str], thumbnail_keys: list[str]) -> int:
        """Delete specific screenshot and thumbnail objects by exact key.

        Unlike delete_screenshots_by_session which wipes an entire prefix,
        this method targets individual object keys for record-level precision.
        Idempotent: silently succeeds if objects are already gone.
        """
        await self.ensure_initialized()
        return await asyncio.to_thread(self._delete_objects_by_key_sync, object_keys, thumbnail_keys)

    def _delete_objects_by_key_sync(self, object_keys: list[str], thumbnail_keys: list[str]) -> int:
        """Synchronous helper for targeted key deletion (runs in thread)."""
        count = 0
        # Full-size screenshots
        unique_keys = list(dict.fromkeys(k for k in object_keys if k))
        if unique_keys:
            delete_list = [DeleteObject(k) for k in unique_keys]
            errors = list(self.client.remove_objects(self._settings.minio_screenshots_bucket, delete_list))
            for err in errors:
                logger.warning("Failed to delete screenshot %s: %s", err.name, err.message)
            count += len(unique_keys)

        # Thumbnails
        unique_thumbs = list(dict.fromkeys(k for k in thumbnail_keys if k))
        if unique_thumbs:
            delete_list = [DeleteObject(k) for k in unique_thumbs]
            errors = list(self.client.remove_objects(self._settings.minio_thumbnails_bucket, delete_list))
            for err in errors:
                logger.warning("Failed to delete thumbnail %s: %s", err.name, err.message)
            count += len(unique_thumbs)

        logger.debug("Deleted %d screenshot objects by exact key", count)
        return count
