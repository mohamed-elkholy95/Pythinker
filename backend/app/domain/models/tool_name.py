...
class ToolName(str, Enum):
    ...
    @classmethod
    def _extract_mcp_tool_name(cls, tool_name: str) -> str | None:
        """Extract the original tool name from an MCP tool name.

        mcp__server__get_docs → get_docs
        mcp_list_resources → mcp_list_resources (built-in, returned as-is)
        """
    ...
    def is_safe_mcp_tool(cls, tool_name: str) -> bool:
        ...
        # Built-in MCP tools (mcp_list_resources, etc.)
        if tool_name.startswith(cls._SAFE_MCP_PREFIXES):
            return True
        # Dynamic MCP server tools (mcp__server__get_docs)
        original = cls._extract_mcp_tool_name(tool_name)
        if original is None:
            return False
        return any(original.startswith(p.removeprefix("mcp_")) for p in cls._SAFE_MCP_PREFIXES)

    @classmethod
    def is_action_mcp_tool(cls, tool_name: str) -> bool:
        """Check if a dynamic MCP tool name is an action tool."""
        if tool_name.startswith(cls._ACTION_MCP_PREFIXES):
            return True
        original = cls._extract_mcp_tool_name(tool_name)
        if original is None:
            return False
        return any(original.startswith(p.removeprefix("mcp_")) for p in cls._ACTION_MCP_PREFIXES)
