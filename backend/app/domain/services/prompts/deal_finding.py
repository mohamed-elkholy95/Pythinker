...
# Research / analytical context suppression — queries about market pricing,
# economic comparisons, or academic analysis should NOT trigger deal intent
# even if they contain price-related keywords like "price comparison".
_RESEARCH_CONTEXT_PATTERN = re.compile(
    r"\b(?:"
    r"research\s+report|comprehensive\s+research|comprehensive\s+report"
    r"|market\s+(?:research|analysis|report|study|overview)"
    r"|comparative\s+(?:analysis|study|report)"
    r"|(?:analyze|examine|investigate|study)\s+(?:the\s+)?(?:market|industry|sector|trend|landscape)"
    r"|(?:across|between)\s+(?:major\s+)?markets"
    r"|economic\s+(?:analysis|comparison|impact)"
    r"|industry\s+(?:report|analysis|overview|landscape)"
    r")\b",
    re.IGNORECASE,
)
...
def detect_deal_intent(text: str) -> bool:
    ...
    A **research context guard** suppresses false positives: queries that
    contain analytical/research phrasing (e.g. "research report",
    "market analysis", "across major markets") are NOT deal intent even if
    they mention "price comparison" or similar keywords.
    ...
    NOT triggered by:
    - "research report about price comparison across major markets"
    - "comprehensive analysis of EV market pricing"
    - "examine the economic impact of pricing trends"
    ...
    # Research context guard — suppress deal detection for analytical queries
    if _RESEARCH_CONTEXT_PATTERN.search(text):
        return False
...
### Report Deliverable (PRIORITY ORDER — follow strictly):
1. **Promo Codes & Coupons Table (TOP PRIORITY)** — present ALL found promo codes
   FIRST at the very top of the report in a professional table:
   | # | Code | Description | Store | Status | Confidence | Expiry | Source | Link |
   - Verified codes first, then unverified
   - Include validation status (Verified/Unverified) and confidence level
   - Show source aggregator and link for each code
   - Add note: "Verify codes at checkout — coupons are time-sensitive."
2. **Best Deal** callout: price + store + savings percentage
3. Price comparison table (Markdown — enables auto Plotly chart):
