...
class ResponseGenerator:
    ...
    def get_artifact_references_section_if_present(self) -> str | None:
        """Return artifact references section only if real artifacts exist.

        Returns None when no artifacts are set or the section would contain
        the boilerplate 'No file artifacts' placeholder.
        """
        section = self._artifact_references_section()
        if section and "No file artifacts" not in section:
            return section
        return None
    ...
    def _default_follow_up_suggestions(self, title: str, content: str) -> list[str]:
        ...
        if topic_hint:
            return [
                f'Can you expand on "{topic_hint}" with an example?',
                f'What are the best next steps for "{topic_hint}"?',
                f'What trade-offs should I consider for "{topic_hint}"?',
            ]
