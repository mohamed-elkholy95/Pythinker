...
2. Adapt your actions based on any changes the user made""",
    ...
    async def end_takeover(
        ...
        # If session is already in a terminal state (completed/failed/cancelled),
        # there's nothing to resume — just clean up takeover state and return success.
        terminal_statuses = {SessionStatus.COMPLETED, SessionStatus.FAILED, SessionStatus.CANCELLED}
        if session.status in terminal_statuses:
            logger.info(
                f"Session {session_id} already in terminal status {session.status.value}; "
                f"skipping resume, transitioning takeover to idle"
            )
            session.takeover_state = TakeoverState.IDLE
            session.takeover_reason = None
            await self._session_repository.save(session)
