...
class AgentTaskFactory:
    ...
    async def create_task(self, session: Session, extra_mcp_configs: dict[str, Any] | None = None) -> Task:
        ...
        # Ownership is derived from acquisition source, not lifecycle mode name:
        # - pooled sandboxes: pool-owned (returned to pool, not destroyed by session)
        # - static-address sandboxes: external/shared (never destroyed)
        # - ad-hoc created containers: session-owned (destroyed on teardown)
        # This mutable flag is updated by acquire_sandbox_from_pool() based on
        # the actual acquisition path taken.
        _acquired_from_pool = False

        def bind_sandbox_metadata(target_sandbox: Sandbox) -> None:
            # Session owns the sandbox unless it came from the pool or a static address
            session_owns = not _acquired_from_pool and not uses_static_sandboxes
            session.sandbox_id = target_sandbox.id
            session.sandbox_owned = session_owns
            session.sandbox_lifecycle_mode = sandbox_lifecycle_mode
            session.sandbox_created_at = datetime.now(UTC) if session_owns else None

        async def acquire_sandbox_from_pool() -> Sandbox:
            nonlocal _acquired_from_pool
        ...
        if not sandbox:
            ...
            # Re-using existing sandbox — keep ownership as already recorded.
            # The sandbox_owned flag was set correctly when first acquired.
            metadata_changed = False
            expected_owned = session.sandbox_owned  # preserve existing ownership
            if session.sandbox_lifecycle_mode != sandbox_lifecycle_mode:
                session.sandbox_lifecycle_mode = sandbox_lifecycle_mode
                metadata_changed = True
            if expected_owned and session.sandbox_created_at is None:
                session.sandbox_created_at = datetime.now(UTC)
                metadata_changed = True
            elif not expected_owned and session.sandbox_created_at is not None:
