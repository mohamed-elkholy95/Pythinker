...
class PlanActFlow(BaseFlow):
    ...
    def _infer_research_depth(self) -> str:
        ...
        Research reports with 4+ steps typically involve multi-source synthesis
        and benefit from expanded grounding context (32K chars) to reduce
        false-positive hallucination flags during verification.
        """
        ...
        while True:
            ...
            try:
                ...
                if self.status == AgentStatus.IDLE:
                    ...
                    _verification_timeout = getattr(settings, "verification_timeout_seconds", 0.0)
                    try:
                        async with (
                    ...
                    # Use self.executor (default agent) which owns the stuck detector
                    ...
                    with trace_ctx.span(
                        ...
                        for attempt in range(max_attempts):
                            ...
                            # Wrap execution with heartbeat interleaving so the
                            # SSE stream stays alive during long LLM calls (e.g.
                            # 75s report generation in a single ask() call).
                            _exec_hb = LLMHeartbeat(
                                phase=PlanningPhase.HEARTBEAT,
                                message=f"Working on step {step.id}...",
                                interval_seconds=3.0,
                            )
                            _exec_inner = step_executor.execute_step(
                                self.plan,
                                step,
                                message,
                            ...
                            async with _exec_hb:
                                async for event in interleave_heartbeat(_exec_inner, _exec_hb):
                                    await self._check_cancelled()
                                    # Skip heartbeat ProgressEvents from interleave — yield them directly
                            ...
                            if (
                                ...
                                logger.warning(f"Step {step.id} stuck recovery exhausted — terminating and advancing")
                                step.success = False
                    ...
                    # Always refresh from session_files when available — the previous
                    # `not self._report_attachments` guard was too conservative and
                    # prevented updates when _report_attachments was stale or empty.
                    if session_files:
                        refreshed = [
                            {
                                "filename": f.filename,
                        ...
                        # Merge: keep existing entries not in session_files, then add fresh ones
                        refreshed_names = {r.get("filename") for r in refreshed}
                        merged = [r for r in self._report_attachments if r.get("filename") not in refreshed_names]
                        merged.extend(refreshed)
                        self._report_attachments = merged
