...
MODEL_PRICING: dict[str, ModelPricing] = {
    ...
    "gemini-2.5-flash": ModelPricing(0.075, 0.30, None),  # Gemini 2.5 Flash (experimental)
    "google/gemini-2.5-flash": ModelPricing(0.075, 0.30, None),  # With provider prefix
    # =========================================
    # Z.AI GLM Models
    # =========================================
    "glm-5-turbo": ModelPricing(0.69, 1.38, 0.35),
    ...
    "moonshot-v1-8k": ModelPricing(1.38, 2.76, None),
    "moonshot-v1-32k": ModelPricing(3.45, 6.90, None),
    "moonshot-v1-128k": ModelPricing(8.27, 8.27, None),
    # =========================================
...
def get_model_pricing(model_name: str) -> ModelPricing:
    ...
    patterns = [
        ...
        (r"^google/gemini-2\.5", "google/gemini-2.5-flash"),
        (r"^llama3\.2", "llama3.2"),
        (r"^llama3\.1", "llama3.1"),
        (r"^llama3", "llama3"),
        (r"^llama2", "llama2"),
        ...
        (r"^glm-5", "glm-5"),
        (r"^glm-4", "glm-4-flash"),
        (r"^kimi", "kimi-for-coding"),
        (r"^moonshot-v1-128k", "moonshot-v1-128k"),
        (r"^moonshot-v1-32k", "moonshot-v1-32k"),
        (r"^moonshot", "moonshot-v1-8k"),
    ]
...
def calculate_cost(
    model_name: str, prompt_tokens: int, completion_tokens: int, cached_tokens: int = 0
) -> tuple[float, float, float]:
...
def select_pricing_snapshot(
    ...
    """Select the effective snapshot for the provider/model at a given time."""
    provider_name = provider.lower().strip()
    matching_snapshots = [
        snapshot
        for snapshot in snapshots
        if snapshot.provider.lower().strip() == provider_name
        and snapshot.matches_model(model_name)
        and snapshot.is_effective_at(at_time)
    ]
    ...
    return max(
        matching_snapshots,
        key=lambda snapshot: (snapshot.effective_from, snapshot.source_retrieved_at),
    )
