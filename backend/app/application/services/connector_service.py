...
class ConnectorService:
    ...
    async def connect_app(
        ...
        For connectors with an mcp_template, credentials are used to build
        a real MCP server configuration. The UserConnector is stored as
        connector_type=CUSTOM_MCP so it flows through the existing MCP pipeline.
        """
        ...
        # Block coming_soon connectors
        if connector.availability == "coming_soon":
            raise ValueError(f"{connector.name} requires OAuth and is not yet available")

        # Check for existing connection
        existing = await self._user_connector_repo.get_by_user_and_connector(user_id, connector_id)
        if existing:
            if existing.status == ConnectorStatus.CONNECTED and not credentials:
                return existing
            # If credentials provided, update the MCP config
            if credentials and connector.mcp_template:
                self._validate_credentials(connector.mcp_template, credentials)
                mcp_config = self._build_mcp_config(connector, credentials)
                existing.mcp_config = mcp_config
                existing.connector_type = ConnectorType.CUSTOM_MCP
                existing.status = ConnectorStatus.PENDING
                existing.error_message = None
                existing.updated_at = datetime.now(UTC)
                updated = await self._user_connector_repo.update(existing.id, existing)
                return updated or existing
            # Re-connect without credentials
        ...
        # MCP-backed app connector
        if connector.mcp_template:
            self._validate_credentials(connector.mcp_template, credentials or {})
            mcp_config = self._build_mcp_config(connector, credentials or {})
        ...
        # Non-MCP app connector (e.g. built-in browser)
        user_connector = UserConnector(
            id=str(uuid.uuid4()),
            user_id=user_id,
            connector_id=connector_id,
    ...
    @staticmethod
    def _validate_credentials(template: McpTemplate, credentials: dict[str, str]) -> None:
        """Validate that all required credential fields are provided."""
        for field in template.credential_fields:
            if field.required and (field.key not in credentials or not credentials[field.key].strip()):
                raise ValueError(f"Missing required credential: {field.label}")

    @staticmethod
    def _build_mcp_config(connector: Connector, credentials: dict[str, str]) -> CustomMcpConfig:
        """Build a CustomMcpConfig from a connector's MCP template and user credentials."""
        template = connector.mcp_template
        if not template:
            raise ValueError("Connector has no MCP template")
        return CustomMcpConfig(
            transport=template.transport,
            command=template.command,
            args=list(template.args),
            env={k: v for k, v in credentials.items() if v.strip()},
            description=connector.description,
        )
