...
class SettingsService:
    ...
    async def get_user_settings(self, user_id: str) -> dict[str, Any]:
        ...
        if settings_doc:
            ...
            return {
                ...
                "api_base": settings_doc.get("api_base", config.api_base),
    ...
    @staticmethod
    def get_live_llm_config() -> dict[str, Any]:
        """Read the actual running LLM model name and API base from the live singleton.

        Returns a dict with 'model_name' and 'api_base' resolved from the
        infrastructure LLM factory, falling back to env var defaults.
        """
        config = get_settings()
        actual_model = config.model_name
        actual_api_base = config.api_base
        try:
            from app.infrastructure.external.llm.factory import get_llm

            llm = get_llm()
            if llm is not None:
                actual_model = getattr(llm, "model_name", actual_model)
