...
class AgentService:
    ...
    async def _record_usage(self, action: str = "record_tool_call", **kwargs: Any) -> str | None:
        """Dispatch run and step usage events to the application usage service."""

        def _require_fields(*required_fields: str) -> None:
            missing = [field for field in required_fields if not kwargs.get(field)]
            if missing:
                formatted_fields = " and ".join(required_fields) if len(required_fields) == 2 else ", ".join(missing)
                raise ValueError(f"{action} requires {formatted_fields}")

        usage_service = get_usage_service()
        if action == "start_run":
            _require_fields("user_id", "session_id")
            run = await usage_service.start_agent_run(
        ...
        if action == "finish_run":
            _require_fields("run_id", "status")
            status_value = kwargs["status"]
            try:
                status = status_value if isinstance(status_value, AgentRunStatus) else AgentRunStatus(status_value)
            except ValueError as exc:
                allowed_values = ", ".join(member.value for member in AgentRunStatus)
                raise ValueError(
                    f"Invalid agent run status: {status_value}. Expected one of: {allowed_values}"
                ) from exc
            await usage_service.finalize_agent_run(
    ...
    async def shell_view(self, session_id: str, shell_session_id: str, user_id: str) -> ShellViewResponse:
        ...
        if result.success and result.data:
            # ShellViewResponse / ConsoleRecord Pydantic validators
            # automatically strip [CMD_BEGIN]/[CMD_END] markers on construction.
