...
from app.application.errors.exceptions import BadRequestError, NotFoundError, UnauthorizedError, ValidationError
...
class AuthService:
    ...
    def _validate_password_complexity(self, password: str) -> tuple[bool, list[str]]:
        ...
        if len(password) > self.settings.password_max_length:
            errors.append(f"Password must be no more than {self.settings.password_max_length} characters long")
        ...
        if self.settings.password_require_special and not any(
            character in self._ASCII_PUNCTUATION for character in password
        ):
    ...
    async def login_with_tokens(self, email: str, password: str, totp_code: str | None = None) -> AuthToken:
        ...
        if not getattr(user, "email_verified", True):
            raise BadRequestError("Email not verified", error_code="email_not_verified")
