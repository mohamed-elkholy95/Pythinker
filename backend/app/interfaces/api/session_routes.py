...
from app.interfaces.dependencies import (
    ...
    get_sandbox_cls,
    get_sandbox_navigation_http_client,
...
def _is_websocket_origin_allowed(websocket: WebSocket) -> bool:
    ...
    # In non-production, auto-accept OrbStack *.orb.local origins (Docker Compose DNS)
    return request_origin in allowed or (
        not getattr(settings, "is_production", False) and request_origin.endswith(".orb.local")
    )
...
async def takeover_navigation_action(
    ...
    try:
        client = await get_sandbox_navigation_http_client()
