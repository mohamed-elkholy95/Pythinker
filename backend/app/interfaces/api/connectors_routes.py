...
def _connector_to_response(c: Connector) -> ConnectorResponse:
    mcp_template_resp = None
    if c.mcp_template:
        mcp_template_resp = McpTemplateResponse(
            credential_fields=[
                CredentialFieldResponse(
                    key=f.key,
                    label=f.label,
                ...
                for f in c.mcp_template.credential_fields
            ],
        )
    ...
    return ConnectorResponse(
        ...
        mcp_template=mcp_template_resp,
    )
...
def _user_connector_to_response(uc: UserConnector) -> UserConnectorResponse:
    ...
    if uc.mcp_config:
        mcp_config_resp = CustomMcpConfigResponse(
            ...
            env={k: _mask_api_key(v) for k, v in uc.mcp_config.env.items()},
...
async def connect_app(
    connector_id: str,
    request: ConnectAppRequest | None = None,
    current_user: User = Depends(get_current_user),
) -> APIResponse[UserConnectorResponse]:
    """Connect a pre-built app connector.

    For connectors with MCP templates, provide credentials in the request body.
    """
    service = get_connector_service()
    credentials = request.credentials if request else None
    try:
        uc = await service.connect_app(current_user.id, connector_id, credentials=credentials)
    except ValueError as e:
