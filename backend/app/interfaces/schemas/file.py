...
class FileInfoResponse(BaseModel):
    ...
    async def from_file_info(file_info: FileInfo) -> Optional["FileInfoResponse"]:
        ...
        if not file_info.file_id:
            logger.warning(f"FileInfo missing file_id: filename={file_info.filename}, file_path={file_info.file_path}")
            return None

        if not file_info.filename:
            logger.warning(f"FileInfo missing filename: file_id={file_info.file_id}, file_path={file_info.file_path}")
            return None
        ...
        try:
            ...
            logger.warning(
                f"File not found when creating signed URL: file_id={file_info.file_id}, filename={file_info.filename}"
            )
            return None
        except Exception as e:
            logger.exception(
                f"Unexpected error converting FileInfo to response: file_id={file_info.file_id}, error={e}"
            )
            return None
