...
def get_email_service() -> EmailService:
    ...
    Uses the runtime Redis (not cache Redis) for OTP code storage.
    OTP codes are critical auth state, not ephemeral cache data.
    """
    logger.info("Creating EmailService instance")
    cache = get_cache()
    # If cache Redis is unavailable, fall back to runtime Redis for OTP storage.
    # OTP verification codes must not be lost — they are auth-critical, not cache.
