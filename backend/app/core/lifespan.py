...
async def _run_periodic_storage_cleanup(
    maintenance_service: MaintenanceService,
    interval_seconds: float = 86400.0,
    screenshot_ttl_days: int = 30,
    snapshot_ttl_days: int = 7,
) -> None:
    """Background task: TTL-based cleanup of old screenshots and snapshots."""
    # Delay first run by 10 minutes to let the system stabilize
    await asyncio.sleep(600)
    while True:
        if not await _try_acquire_leader_lock("storage_cleanup", ttl_seconds=int(interval_seconds) - 5):
            await asyncio.sleep(interval_seconds)
            continue
        try:
            screenshot_result = await maintenance_service.cleanup_old_screenshots(ttl_days=screenshot_ttl_days)
            snapshot_result = await maintenance_service.cleanup_old_snapshots(ttl_days=snapshot_ttl_days)
            total = screenshot_result.get("documents_deleted", 0) + snapshot_result.get("documents_deleted", 0)
            if total > 0:
                logger.info(
                    "Periodic storage cleanup: %d screenshots, %d snapshots deleted",
                    screenshot_result.get("documents_deleted", 0),
                    snapshot_result.get("documents_deleted", 0),
                )
        except Exception as e:
            logger.warning("Periodic storage cleanup failed: %s", e)
        await asyncio.sleep(interval_seconds)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            users_col = get_mongodb().client[settings.mongodb_database].users
            result = await users_col.update_many(
                {"email_verified": {"$exists": False}},
                {"$set": {"email_verified": True}},
            )
            if result.modified_count > 0:
                logger.info("Migration: set email_verified=True for %d existing users", result.modified_count)
        except Exception as e:
            logger.warning("email_verified migration failed (non-critical): %s", e)
        ...
        try:
            ...
            if await _try_acquire_startup_singleton("storage_cleanup_loop"):
                storage_cleanup_task = asyncio.create_task(
                    _run_periodic_storage_cleanup(
                        maintenance_service,
                        interval_seconds=settings.storage_cleanup_interval_seconds,
                        screenshot_ttl_days=settings.screenshot_ttl_days,
                        snapshot_ttl_days=settings.sandbox_snapshot_ttl_days,
                    ),
                )
                logger.info("Periodic storage cleanup background task started")
        except Exception as e:
        ...
        if sandbox_pool_enabled:
            try:
                ...
                pool = await start_sandbox_pool(DockerSandbox)
                _health_state["sandbox_pool"] = True

                # Wire session-aware orphan protection so the reaper
                # never kills containers still leased by active sessions.
                async def _active_sandbox_ids() -> set[str]:
                    try:
                        _db = get_mongodb().client[settings.mongodb_database]
                        cursor = _db.sessions.find(
                            {
                                "sandbox_id": {"$ne": None},
                                "status": {"$in": ["running", "initializing", "pending", "paused", "completed"]},
                            },
                            {"sandbox_id": 1},
                        )
                        return {doc["sandbox_id"] async for doc in cursor if doc.get("sandbox_id")}
                    except Exception as exc:
                        logger.debug("Failed to fetch active sandbox IDs: %s", exc)
                        return set()

                pool.set_active_sandbox_provider(_active_sandbox_ids)
