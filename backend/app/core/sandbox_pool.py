...
def _extract_container_info(container: Any, now: float, *, has_label: bool) -> tuple[str, str, float, bool]:
    """Extract (name, short_id, created_epoch, has_label) from a Docker container object."""
    name = container.name
    ...
    return (name, container.id[:12], created_epoch, has_label)
...
class SandboxPool:
    ...
    async def _reap_orphan_containers(self) -> int:
        ...
        Uses Docker labels (pythinker.managed=true) as primary filter, with
        name-prefix as a fallback for containers created before labels were added.
        Skips containers still leased by active sessions.
        ...
        if self._active_sandbox_provider:
            try:
                ...
                logger.debug("Failed to fetch active session sandbox IDs: %s", e)

        def _find_candidates() -> list[tuple[str, str, float, bool]]:
            """Find Pythinker-managed containers. Returns (name, id, created_epoch, has_label)."""
            results = []
            try:
                dc = docker.from_env()
                # Primary: label-based discovery
                labeled = dc.containers.list(all=True, filters={"label": "pythinker.managed=true"})
                seen_ids: set[str] = set()
                for container in labeled:
                    seen_ids.add(container.id)
                    results.append(_extract_container_info(container, now, has_label=True))

                # Fallback: name-prefix for containers without labels (backward compat)
                prefixed = dc.containers.list(all=True, filters={"name": prefix})
                results.extend(
                    _extract_container_info(c, now, has_label=False) for c in prefixed if c.id not in seen_ids
                )
            except Exception as e:
                logger.debug("Orphan scan failed: %s", e)
            return results
        ...
        for name, container_id, created_epoch, has_label in candidates:
            # Skip containers younger than grace period
            if now - created_epoch < self._reaper_grace_period:
                logger.debug("Reaper skip %s: younger than grace period (%.0fs old)", name, now - created_epoch)
                continue
            # Skip containers tracked by the pool or held by active sessions
            if name in known_ids or container_id in known_ids:
                logger.debug("Reaper skip %s: still leased by pool or active session", name)
                continue
            # Destroy orphan
            try:
                ...
                reaped += 1
                source = "label" if has_label else "prefix"
                logger.info("Reaped orphan sandbox container: %s (%s, discovered via %s)", name, container_id, source)
            except Exception as e:
                logger.debug("Failed to reap orphan %s: %s", name, e)

        if reaped:
            logger.info("Orphan reaper: cleaned up %d containers", reaped)
        return reaped

    # --- Priority 3: Health Monitoring ---

    async def _hard_destroy_sandbox(self, sandbox: "Sandbox") -> None:
        """Best-effort sandbox destruction for pool-side crash cleanup.

        Matches the session teardown pattern: bounded destroy with
        force_destroy fallback. Treats container-not-found as success.
        """
        try:
            await asyncio.wait_for(sandbox.destroy(), timeout=15.0)
        except TimeoutError:
            logger.warning(
                "Sandbox %s destroy timed out, attempting force_destroy", getattr(sandbox, "container_id", sandbox.id)
            )
            try:
                if hasattr(sandbox, "force_destroy"):
                    await asyncio.wait_for(sandbox.force_destroy(), timeout=10.0)
            except Exception:
                logger.debug("Force destroy also failed for sandbox %s", getattr(sandbox, "container_id", sandbox.id))
        except DockerNotFound:
            pass  # Already gone — success
        except Exception as e:
            logger.debug("Failed to destroy crashed sandbox %s: %s", getattr(sandbox, "container_id", sandbox.id), e)
            ...
            try:
                ...
                for failed_sandbox in failed_sandboxes:
                    try:
                        ...
                        # Clean up the failed sandbox using the real lifecycle API
                        await self._hard_destroy_sandbox(failed_sandbox)
                    except Exception as e:
