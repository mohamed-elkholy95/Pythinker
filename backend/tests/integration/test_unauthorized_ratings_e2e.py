...
async def test_unauthorized_rating_blocked_e2e():
    ...
    # Mock repository
    with patch("app.interfaces.dependencies.get_current_user", return_value=user1):
        with patch("app.interfaces.dependencies.get_session_repository") as mock_repo:
...
async def test_authorized_rating_succeeds_e2e():
    ...
    with patch("app.interfaces.dependencies.get_current_user", return_value=user1):
        with patch("app.interfaces.dependencies.get_session_repository") as mock_repo:
            with patch("app.interfaces.dependencies.get_rating_service") as mock_rating:
                with patch("app.interfaces.dependencies.get_email_service") as mock_email:
                    # Setup mocks
                    mock_repo_instance = Mock(spec=MongoSessionRepository)
