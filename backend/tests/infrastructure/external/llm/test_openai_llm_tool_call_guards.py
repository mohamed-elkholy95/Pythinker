...
@pytest.mark.asyncio
async def test_ask_times_out_slow_non_tool_glm_calls_with_hard_timeout() -> None:
    llm = _build_llm()
    ...
    with (
        patch(
            ...
            return_value=SimpleNamespace(
                llm_tool_max_tokens=128,
                llm_tool_request_timeout=0.0,
                llm_tool_timeout_max_retries=0,
                llm_request_timeout=300.0,
                llm_glm_hard_call_timeout=0.01,
                llm_hard_call_timeout=0.0,
                fast_model="",
                llm_slow_request_threshold=30.0,
            ),
        ),
        ...
        await llm.ask(messages=messages, tools=None)
