...
def _mock_search_deals_settings() -> SimpleNamespace:
    """Settings stub for adapter.search_deals() (v2 unified path)."""
    return SimpleNamespace(
        deal_search_mode="shopping",
        deal_verify_top_n=0,
        deal_verify_timeout=5.0,
...
@pytest.mark.asyncio
async def test_search_deals_sets_no_matches_reason_when_shopping_returns_empty(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """If Shopping API and web fallback both return 0 results, empty_reason should be NO_MATCHES."""
    monkeypatch.setattr(adapter_module, "get_settings", lambda: _mock_search_deals_settings())

    search_engine = AsyncMock()
    adapter = DealFinderAdapter(scraper=AsyncMock(), search_engine=search_engine)

    # Both Shopping and web search return nothing
    adapter._search_via_shopping = AsyncMock(return_value=[])  # type: ignore[method-assign]
    adapter._search_via_web = AsyncMock(return_value=[])  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_search_deals_sets_no_matches_reason_with_explicit_stores(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Explicit stores use the v2 path. When Shopping returns no results, empty_reason is NO_MATCHES."""
    monkeypatch.setattr(adapter_module, "get_settings", lambda: _mock_search_deals_settings())

    search_engine = AsyncMock()
    adapter = DealFinderAdapter(scraper=AsyncMock(), search_engine=search_engine)

    # v2 path: Shopping and web fallback both return nothing
    adapter._search_via_shopping = AsyncMock(return_value=[])  # type: ignore[method-assign]
    adapter._search_via_web = AsyncMock(return_value=[])  # type: ignore[method-assign]
