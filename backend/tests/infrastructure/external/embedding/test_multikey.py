...
class TestEmbeddingMultiKey:
    ...
    async def test_embedding_works_without_redis(self):
        """Test in-memory mode without Redis — pool still returns healthy keys.

        Note: The key pool may lazily acquire a RedisClient singleton via
        _ensure_redis(), so we verify functional behavior (key retrieval works)
        rather than internal state (_redis is None).
        """
        client = EmbeddingClient(
            api_key="test-key-1",
        ...
        # Pool works regardless of whether lazy Redis acquisition succeeded
        assert client._key_pool is not None
