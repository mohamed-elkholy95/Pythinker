"""Tests for ChartTool HTML + PNG output and chart type support."""
...
@pytest.mark.asyncio
async def test_chart_tool_includes_html_output_path_in_script_input() -> None:
    sandbox = AsyncMock()
    sandbox.file_write = AsyncMock(return_value=MagicMock(success=True))
    sandbox.exec_command = AsyncMock(
        return_value=MagicMock(
            success=True,
            data={"status": "completed", "output": _build_success_output()},
            message="",
        )
    )
    ...
    assert result.data.get("html_path") == "/home/ubuntu/chart-abc12345.html"
    assert result.data.get("html_size") == 48000
    assert result.data.get("png_path") == "/home/ubuntu/chart-abc12345.png"

    write_call = sandbox.file_write.call_args
    assert write_call is not None
    written_spec = json.loads(write_call.kwargs["content"])
    assert "output_html" in written_spec
    assert str(written_spec["output_html"]).endswith(".html")
    assert "output_png" in written_spec
    assert "output_json" not in written_spec  # No JSON output
