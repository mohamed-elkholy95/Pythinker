...
async def test_browse_top_results_avoids_duplicate_navigation_during_race() -> None:
    ...
    real_url_calls = [c for c in mock_browser.navigate_for_display.call_args_list if c[0][0] != "about:blank"]
    assert len(real_url_calls) == 1, (
        f"Expected 1 real URL navigation, got {len(real_url_calls)}: {[c[0][0] for c in real_url_calls]}"
    )
