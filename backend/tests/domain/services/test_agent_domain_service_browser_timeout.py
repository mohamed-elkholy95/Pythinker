...
async def test_create_task_recycles_sandbox_on_browser_timeout():
    ...
    with patch("app.domain.services.agent_domain_service.get_settings", return_value=settings), patch(
        "app.domain.services.agent_domain_service.asyncio.wait_for", side_effect=fake_wait_for
    ):
