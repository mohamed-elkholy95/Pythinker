...
class TestRewriteChartImageUrls:
    ...
    def test_empty_content_no_error(self):
        event = self._make_report(
            content="",
            attachments=[FileInfo(file_id="x", filename="comparison-chart-report-1.png", content_type="image/png")],
        )
        AgentTaskRunner._rewrite_chart_image_urls(event)
    ...
    def test_canonical_fallback_when_filename_differs(self):
        """After sync, the attachment filename may differ from the markdown reference.

        The markdown uses comparison-chart-{id}.png but after MinIO sync the
        attachment filename becomes the sandbox path basename (e.g. slugified title).
        The fallback should match using the canonical comparison-chart-{id} pattern.
        """
        # Simulate: markdown has comparison-chart-report-1.png but synced
        # attachment filename is the sandbox path basename
        attachment = FileInfo(
            file_id="minio-abc",
            filename="top_5_python_web_frameworks.png",  # sandbox path basename
            content_type="image/png",
        )
        event = self._make_report(
            content="## Charts\n![Comparison Chart](comparison-chart-report-1.png)\nMore text.",
        ...
        AgentTaskRunner._rewrite_chart_image_urls(event)
        assert "](/api/v1/files/minio-abc/download)" in event.content
        assert "comparison-chart-report-1.png" not in event.content
