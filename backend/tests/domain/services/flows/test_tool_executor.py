...
async def test_heartbeat_emitted_during_slow_tool():
    ...
    events = []
    async for event in executor.execute("browser_navigate", slow_tool):
        events.append(event)
...
async def test_tool_result_yielded_after_heartbeats():
    ...
    events = []
    async for event in executor.execute("search", fast_tool):
        events.append(event)
...
async def test_fast_tool_no_heartbeats():
    ...
    events = []
    async for event in executor.execute("quick_tool", instant_tool):
        events.append(event)
