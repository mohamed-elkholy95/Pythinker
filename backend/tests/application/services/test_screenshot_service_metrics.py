...
async def test_query_service_records_fetch_success_metrics():
    ...
    mongodb = SimpleNamespace(get_screenshot=AsyncMock(return_value=b"served-bytes"))
    service = ScreenshotQueryService(repository=repository, mongodb=mongodb)

    image_data = await service.get_image_bytes(
        session_id="session-metrics-3",
...
async def test_query_service_records_fetch_error_metrics_for_missing_screenshot():
    ...
    mongodb = SimpleNamespace(get_screenshot=AsyncMock())
    service = ScreenshotQueryService(repository=repository, mongodb=mongodb)
