...
class TestGenerateResearchQueries:
    """Test _generate_research_queries method."""

    @pytest.fixture
    def flow(
        self,
    ):
        """Create a minimal PlanActFlow for testing query generation."""
        from app.domain.services.flows.plan_act import PlanActFlow

        # Create with minimal mocks
        flow = MagicMock(spec=PlanActFlow)
        # Use the real method
        flow._generate_research_queries = PlanActFlow._generate_research_queries.__get__(flow, PlanActFlow)
        return flow

    def test_generates_queries_with_variations(self, flow):
        """Test that query generation includes topic and variations."""
        topic = "LLM pricing comparison"
        queries = flow._generate_research_queries(topic)

        # Should include original topic
    ...
    def test_does_not_duplicate_best(self, flow):
        """Test that 'best' is not added if already present."""
        topic = "best LLM for coding"
        queries = flow._generate_research_queries(topic)

        # Should not have "best best LLM..."
        best_count = sum(1 for q in queries if q.lower().startswith("best best"))
        assert best_count == 0

    def test_does_not_duplicate_comparison(self, flow):
        """Test that 'comparison' is not added if already present."""
        topic = "LLM comparison 2026"
        queries = flow._generate_research_queries(topic)

        # Should not have "LLM comparison 2026 comparison"
        comparison_duplicates = sum(1 for q in queries if q.lower().count("comparison") > 1)
        assert comparison_duplicates == 0
