...
class TestOrphanReaper:
    """Tests for _reap_orphan_containers() with session-aware protection."""

    @pytest.mark.asyncio
    @patch(SETTINGS_PATH, return_value=_make_mock_settings())
    async def test_reaper_skips_container_leased_by_active_session(self, mock_settings: MagicMock) -> None:
        """Container acquired from pool and held by a session is not reaped."""
        mock_settings.return_value.sandbox_name_prefix = "pythinker-sandbox"

        pool = SandboxPool(_make_mock_sandbox_cls())
        pool._reaper_grace_period = 0  # No grace period for test

        # Simulate a container that left the pool but is leased by a session
        leased_name = "pythinker-sandbox-abc12345"
        mock_container = MagicMock()
        mock_container.name = leased_name
        mock_container.id = "abcdef123456full"
        mock_container.attrs = {"Created": "2020-01-01T00:00:00Z"}
        mock_container.labels = {"pythinker.managed": "true"}
        mock_container.status = "running"

        mock_dc = MagicMock()
        mock_dc.containers.list.return_value = [mock_container]

        # Wire session-aware provider: the container is leased by a session
        async def _provider() -> set[str]:
            return {leased_name}

        pool.set_active_sandbox_provider(_provider)

        with patch("app.core.sandbox_pool.docker.from_env", return_value=mock_dc):
            reaped = await pool._reap_orphan_containers()

        assert reaped == 0  # Not reaped because it's leased

    @pytest.mark.asyncio
    @patch(SETTINGS_PATH, return_value=_make_mock_settings())
    async def test_reaper_destroys_orphan_without_session_lease(self, mock_settings: MagicMock) -> None:
        """Container not in pool and not leased by any session IS reaped."""
        mock_settings.return_value.sandbox_name_prefix = "pythinker-sandbox"

        pool = SandboxPool(_make_mock_sandbox_cls())
        pool._reaper_grace_period = 0

        orphan_name = "pythinker-sandbox-orphan99"
        mock_container = MagicMock()
        mock_container.name = orphan_name
        mock_container.id = "orphanfullid1234"
        mock_container.attrs = {"Created": "2020-01-01T00:00:00Z"}
        mock_container.labels = {"pythinker.managed": "true"}
        mock_container.status = "running"

        mock_dc = MagicMock()
        # Label filter returns the container, prefix filter returns empty (already found)
        mock_dc.containers.list.side_effect = [
            [mock_container],  # label query
            [],  # prefix query (no additional)
        ]
        # Also used for force removal
        mock_dc.containers.get.return_value = mock_container

        # No active sessions
        async def _empty_provider() -> set[str]:
            return set()

        pool.set_active_sandbox_provider(_empty_provider)
    ...
    @pytest.mark.asyncio
    @patch(SETTINGS_PATH, return_value=_make_mock_settings())
    async def test_reaper_skips_young_container(self, mock_settings: MagicMock) -> None:
        """Container younger than grace period is not reaped."""
        mock_settings.return_value.sandbox_name_prefix = "pythinker-sandbox"

        pool = SandboxPool(_make_mock_sandbox_cls())
        pool._reaper_grace_period = 9999  # Very long grace period

        mock_container = MagicMock()
        mock_container.name = "pythinker-sandbox-young1"
        mock_container.id = "youngfullid12345"
        # Created just now — will be within grace period
        from datetime import UTC, datetime

        mock_container.attrs = {"Created": datetime.now(UTC).isoformat()}
        mock_container.labels = {"pythinker.managed": "true"}
