...
from .agent import A2APythinker
from .agent_executor import PythinkerExecutor
...
async def main(host: str = "localhost", port: int = 10000):
    """Starts the Pythinker Agent server."""
    try:
        ...
        agent_card = AgentCard(
            name="Pythinker Agent",
            description="A versatile agent that can solve various tasks using multiple tools including MCP-based tools",
            url=f"http://{host}:{port}/",
            version="1.0.0",
            defaultInputModes=A2APythinker.SUPPORTED_CONTENT_TYPES,
            defaultOutputModes=A2APythinker.SUPPORTED_CONTENT_TYPES,
        ...
        request_handler = DefaultRequestHandler(
            agent_executor=PythinkerExecutor(
                agent_factory=lambda: A2APythinker.create(max_steps=3)
            ),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Start Pythinker Agent service")
    parser.add_argument(
        "--host",
        type=str,
        default="localhost",
