...
class SandboxVisionTool(SandboxToolsBase):
    ...
    parameters: dict = {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["see_image"],
                "description": "， see_image",
            },
            "file_path": {
                "type": "string",
                "description": " /workspace ， 'screenshots/image.png'",
            },
        },
    ...
    async def execute(
        ...
        """
        if action != "see_image":
            return self.fail_response(f": {action}")
        if not file_path:
            return self.fail_response("file_path ")
        try:
            ...
            try:
                ...
                if file_info.is_dir:
                    return self.fail_response(f" '{cleaned_path}' ，")
            except Exception:
                return self.fail_response(f": '{cleaned_path}'")
            if file_info.size > MAX_IMAGE_SIZE:
                return self.fail_response(
                    f" '{cleaned_path}'  ({file_info.size / (1024*1024):.2f}MB)， {MAX_IMAGE_SIZE / (1024*1024)}MB"
                )
            try:
            ...
            if not mime_type or not mime_type.startswith("image/"):
                ...
                if ext == ".jpg" or ext == ".jpeg":
                    ...
                    return self.fail_response(
                        f": '{cleaned_path}': JPG, PNG, GIF, WEBP"
                    )
            ...
            if len(compressed_bytes) > MAX_COMPRESSED_SIZE:
                return self.fail_response(
                    f" '{cleaned_path}'  ({len(compressed_bytes) / (1024*1024):.2f}MB)， {MAX_COMPRESSED_SIZE / (1024*1024)}MB"
                )
            ...
            # return self.success_response(f" '{cleaned_path}' ( {file_info.size / 1024:.1f}KB  {len(compressed_bytes) / 1024:.1f}KB)")
            return ToolResult(
                output=f" '{cleaned_path}'",
                base64_image=base64_image,
            )
        except Exception as e:
            return self.fail_response(f"see_image : {str(e)}")
