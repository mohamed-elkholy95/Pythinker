...
async def update_user_skill(skill_id: str, req: UpdateUserSkillRequest, request: Request):
    user = _get_current_user(request)
    result = skill_store.update_user_skill(user["id"], skill_id, req.enabled, req.config, req.order)
    if not result:
...
async def get_skill_package(package_id: str):
    return _wrap({
        ...
        "files": [{"path": "src/main.py", "content": "# Demo skill\nprint('hello')", "size": 256}],
        "file_count": 1,
...
async def download_skill_package(package_id: str):
    ...
    return Response(content=b"PK\x03\x04mock_skill_package", media_type="application/zip",
...
async def get_skill_package_file(package_id: str, path: str = ""):
    return _wrap({"path": path, "content": "# Demo skill file content\nprint('hello')", "size": 128})
