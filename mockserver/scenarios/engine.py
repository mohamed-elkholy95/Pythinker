...
def select_scenario(message: str, deep_research: bool = False):
    ...
    if "wide research" in msg:
        ...
        return run
    if any(
        kw in msg for kw in ["research", "find", "search", "look up", "investigate"]
    ):
    ...
    if any(
        kw in msg
        for kw in ["run", "execute", "install", "pip", "npm", "command", "terminal"]
    ):
        from scenarios.shell_execution import run

        return run
    if any(
        kw in msg for kw in ["write", "create", "build", "make", "generate", "code"]
    ):
...
async def run_scenario(
    message: str, session_id: str, deep_research: bool = False
