...
| `API_BASE` | `https://api.openai.com/v1` | Yes | Base API address for specifying model service endpoint |

### Model Configuration

| Configuration | Default Value | Required | Description |
|---------------|---------------|----------|-------------|
| `MODEL_NAME` | `gpt-4o` | Yes | Name of the model to use |
...
| `MONGODB_DATABASE` | `pythinker` | No | Database name |
...
| `SANDBOX_IMAGE` | `pythinker/pythinker-sandbox` | No | Docker sandbox image name |
| `SANDBOX_NAME_PREFIX` | `sandbox` | No | Sandbox container name prefix |
| `SANDBOX_TTL_MINUTES` | `30` | No | Sandbox time-to-live in minutes |
| `SANDBOX_NETWORK` | `pythinker-network` | No | Docker network name |
